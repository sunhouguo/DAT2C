...
const std::string strDefaultCfg = "S101Cfg.xml";
size_t CS101::S101ObjectCounter_ = 1;
...
const size_t NormalYkFrameLength = 15;

CS101::CS101(boost::asio::io_service & io_service)
...
int CS101::ConnectSubYkSig(share_commpoint_ptr point)
{
	if (point)
	{
		if (point->getCommPointType() == PRISTATION_NODE)
		{
			return point->ConnectCmdRelaySig(boost::bind(&CS101::ProcessSubYkSig,this,_1,_2,_3,_4));
		}
	}
...
int CS101::DisconnectSubYkSig( share_commpoint_ptr point,bool bForceClose )
{
	if (point)
	{
...
void CS101::ProcessSubYkSig(typeCmd cmdType,unsigned char ReturnCode,share_commpoint_ptr point,boost::any val)
{
	if (!point)
	{
		return;
...
int CS101::LoadXmlCfg(std::string filename)
{
...
void CS101::SaveXmlCfg(std::string filename)
{
	FileSystem::CMarkup xml;
...
void CS101::InitObjectIndex()
{
	ProtocolObjectIndex_ = S101ObjectCounter_++;
}

int CS101::InitProtocol()
{
	ConnectSubAliveSig();
	...
	AddStatusLogWithSynT("S101规约的通道打开成功。\n");

	return 0;
}

void CS101::UninitProtocol()
{
	DisconnectSubAliveSig();
	
	CProtocol::UninitProtocol();

	AddStatusLogWithSynT("S101规约的通道关闭成功。\n");
}

int CS101::MatchFrameHead( size_t & exceptedBytes )
{
...
int CS101::CheckFrameHead(unsigned char * buf,size_t & exceptedBytes)
{
	if ((buf[0] == 0x68) && (buf[3] == 0x68) && (buf[1] == buf[2]))
	{
		...
		if (CheckPubKey())
		{
			...
			//std::ostringstream ostr;
			//ostr<<"FrameTypeLocation_ = "<<FrameTypeLocation_<<std::endl;
			//ostr<<"FrameTypeLength_ = "<<FrameTypeLength_<<std::endl;
			//ostr<<"buf["<<FrameTypeLocation_<<"] = "<<(short)buf[FrameTypeLocation_]<<std::endl;
			//ostr<<"FrameType = "<<FrameType<<std::endl;
			//PrintDebug(ostr.str());
			...
			if (FrameType == M_SC_NA_1 || FrameType == M_DC_NA_1)
			{
				...
				//std::ostringstream ostr;
				//ostr<<"exceptedBytes with key = "<<exceptedBytes<<std::endl;
				//PrintDebug(ostr.str());
			}
		}

		//std::ostringstream ostr;
		//ostr<<"exceptedBytes = "<<exceptedBytes<<std::endl;
		//PrintDebug(ostr.str());
...
int CS101::CheckFrameTail(unsigned char * buf,size_t exceptedBytes)
{
	size_t sum = 0;
	...
	{
		return CheckPlusTail(buf,exceptedBytes);
	}

	return -1;
}

int CS101::ParseShortFrame(unsigned char * buf, share_pristation_ptr pristationPtr)
{
...
int CS101::ParseLongFrame(unsigned char * buf, share_pristation_ptr pristationPtr,size_t exceptedBytes)
{
	...
	case M_SC_NA_1: //single yk
		{
			if (CheckPubKey())
			{
				if (decrypt(getSignStartIndex(),exceptedBytes - getSignStartIndex(),buf[1] + 6 - getSignStartIndex(),buf))
				{
					std::ostringstream ostr;
...
//遥控数字签名验证相关接口
int CS101::ParseYKKeyError(unsigned char * buf,share_pristation_ptr pristationPtr,size_t TransReason,unsigned char Data_Code,bool DoubleFlag)
{
	//在这里进行遥控加密异常的处理
...
int CS101::ParseSingleYKSel(unsigned char * buf,share_pristation_ptr pristationPtr)
{
...
int CS101::ParseSingleYKExe(unsigned char * buf,share_pristation_ptr pristationPtr)
{
	int yk_no = BufToVal(&buf[InfoAddrLocation_],InfoAddrLength_) - SYK_START_ADDR_;
	unsigned char yk_type = buf[DataLocation_] & 0x03;
...
int CS101::ParseSingleYKCancel(unsigned char * buf,share_pristation_ptr pristationPtr)
{
	int yk_no = BufToVal(&buf[InfoAddrLocation_],InfoAddrLength_) - SYK_START_ADDR_;
	unsigned char yk_type = buf[DataLocation_] & 0x03;
...
int CS101::ParseDoubleYkSel(unsigned char * buf,share_pristation_ptr pristationPtr)
{
...
int CS101::ParseDoubleYkExe(unsigned char * buf,share_pristation_ptr pristationPtr)
{
	int yk_no = BufToVal(&buf[InfoAddrLocation_],InfoAddrLength_) - DYK_START_ADDR_;
	unsigned char yk_type = buf[DataLocation_] & 0x03;
...
int CS101::ParseDoubleYkCancel(unsigned char * buf,share_pristation_ptr pristationPtr)
{
	int yk_no = BufToVal(&buf[InfoAddrLocation_],InfoAddrLength_) - DYK_START_ADDR_;
	unsigned char yk_type = buf[DataLocation_] & 0x03;
...
int CS101::ParseCallAllData(unsigned char * buf,share_pristation_ptr pristationPtr)
{
	QOI_ = buf[DataLocation_];

	AddWaitCmdVal(CALL_ALL_DATA_CON,CALL_ALL_DATA_CON_PRIORITY,pristationPtr);
	AddSendCmdVal(CONFIRM_ACK,CONFIRM_ACK_PRIORITY,pristationPtr);
...
int CS101::ParseCallAllYMData(unsigned char * buf,share_pristation_ptr pristationPtr)
{
	QOI_ = buf[DataLocation_];

	AddWaitCmdVal(CALL_ALL_YM_CON,CALL_ALL_DD_CON_PRIORITY,pristationPtr);
	AddSendCmdVal(CONFIRM_ACK,CONFIRM_ACK_PRIORITY,pristationPtr);

	return 0;
}

int CS101::ParseSynTime(unsigned char * buf,share_pristation_ptr pristationPtr)
{
	...
	AddWaitCmdVal(SYN_TIME_CON,SYN_TIME_CON_PRIORITY,pristationPtr);
	AddSendCmdVal(CONFIRM_ACK,CONFIRM_ACK_PRIORITY,pristationPtr);
...
int CS101::ParseResetLink(unsigned char * buf, share_pristation_ptr pristationPtr)
{
	AddSendCmdVal(CONFIRM_ACK,CONFIRM_ACK_PRIORITY,pristationPtr);
...
int CS101::ParseRequireLink(unsigned char * buf, share_pristation_ptr pristationPtr)
{
...
int CS101::ParseCallPrimaryDatas(unsigned char * buf, share_pristation_ptr pristationPtr)
{
	CCmd waitVal;
...
int CS101::ParseCallSecondaryDatas(unsigned char * buf, share_pristation_ptr pristationPtr)
{
	CCmd waitVal;
	DataBase::CCosPoint cosVal;
...
int CS101::ParseFrameBody(unsigned char * buf,size_t exceptedBytes)
{
	int ret = 0;
...
unsigned char CS101::getACD(share_pristation_ptr pristationPtr)
{
...
int CS101::CheckACD(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
...
unsigned char CS101::getFCB(share_pristation_ptr pristationPtr)
{
...
int CS101::setFCB(share_pristation_ptr pristationPtr,unsigned char val)
{
	val = val & ACT_FCB;
	if (val == ACT_FCB)
	{
...
int CS101::CheckFCB( unsigned char FCV,unsigned char FCB,typeCmd CurCmd,share_pristation_ptr pristationPtr )
{
	int ret = 0;
...
int CS101::getAddrByRecvFrame(unsigned char * buf)
{
	unsigned char funcType = buf[0];
...
int CS101::AssembleLinkStatus(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;

	count += ValToBuf(&buf[count],M_LKR_NA_3 | DIR_PRM,1);
	count += ValToBuf(&buf[count],pristationPtr->getAddr(),AsduAddrLength_);

	return count - bufIndex;
}

int CS101::AssembleConfirmNack(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;

	count += ValToBuf(&buf[count],M_NV_NA_3 | DIR_PRM,1);
	count += ValToBuf(&buf[count],pristationPtr->getAddr(),AsduAddrLength_);

	return count - bufIndex;
}

int CS101::AssembleConfirmAck(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;

	count += ValToBuf(&buf[count],M_CON_NA_3 | DIR_PRM,1);
	count += ValToBuf(&buf[count],pristationPtr->getAddr(),AsduAddrLength_);

	return count - bufIndex;
}

int CS101::AssembleDoubleYKSelCon(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no, unsigned char yk_code,unsigned char trans_reason)
{
	size_t count = bufIndex;

	count += ValToBuf(&buf[count],M_AV_NA_3 | DIR_PRM,1);
	count += ValToBuf(&buf[count],pristationPtr->getAddr(),AsduAddrLength_);
...
int CS101::AssembleDoubleYKExeCon(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no, unsigned char yk_code,unsigned char trans_reason)
{
	size_t count = bufIndex;
...
int CS101::AssembleDoubleYKCancelCon(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no,unsigned char yk_code,unsigned char trans_reason)
{
	size_t count = bufIndex;
...
int CS101::AssembleDoubleYKOver(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no,unsigned char yk_code)
{
	size_t count = bufIndex;
...
int CS101::AssembleSingleYKSelCon(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no, unsigned char yk_code,unsigned char trans_reason)
{
	size_t count = bufIndex;
...
int CS101::AssembleSingleYKExeCon(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no, unsigned char yk_code,unsigned char trans_reason)
{
	size_t count = bufIndex;
...
int CS101::AssembleSingleYKCancelCon(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no,unsigned char yk_code,unsigned char trans_reason)
{
	size_t count = bufIndex;
...
int CS101::AssembleSingleYKOver(size_t bufIndex, unsigned char * buf, share_pristation_ptr pristationPtr,unsigned short yk_no,unsigned char yk_code)
{
	size_t count = bufIndex;
...
int CS101::AssembleSynTimeCon(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,boost::posix_time::ptime time)
{
	size_t count = bufIndex;
...
int CS101::AssembleCallDataCon(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;
...
int CS101::AssembleCallDataOver(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;
...
int CS101::AssembleAllSingleYX( size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,int & startIndex,size_t info_num )
{
	size_t count = bufIndex;
...
int CS101::AssembleAllDoubleYX( size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,int & startIndex,size_t info_num )
{
	size_t count = bufIndex;
...
int CS101::AssembleAllYCWithVaild( size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,int & startIndex,size_t info_num )
{
	size_t count = bufIndex;
...
int CS101::AssembleCallYMCon(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;
...
int CS101::AssembleCallYMOver(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;
...
int CS101::AssembleAllYM( size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,int & startIndex,size_t info_num )
{
	size_t count = bufIndex;
...
int CS101::AssembleSingleCOS(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,DataBase::CCosPoint * cosBuf,size_t info_num)
{
	size_t count = bufIndex;
...
int CS101::AssembleDoubleCOS(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,DataBase::CCosPoint * cosBuf,size_t info_num)
{
	size_t count = bufIndex;
...
int CS101::AssembleSingleSOE(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,DataBase::CSoePoint * soeBuf,size_t info_num)
{
	size_t count = bufIndex;
...
int CS101::AssembleDoubleSOE(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,DataBase::CSoePoint * soeBuf,size_t info_num)
{
	size_t count = bufIndex;
...
int CS101::AssembleYcVar(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,DataBase::CYcVarPoint * ycvarBuf,size_t info_num)
{
	size_t count = bufIndex;
...
int CS101::AssembleYcVarWithVaild(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr,DataBase::CYcVarPoint * ycvarBuf,size_t info_num)
{
	size_t count = bufIndex;
...
int CS101::AssembleEndInit(size_t bufIndex,unsigned char * buf,share_pristation_ptr pristationPtr)
{
	size_t count = bufIndex;
...
int CS101::AssembleFrameHead(size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
	size_t count = bufIndex;
...
int CS101::AssembleFrameBody(size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
	int bytesAssemble = 0;
	...
	case YK_EXE_CON:
		{
			...
			if (bytesAssemble > 0)
			{
				if (yk_para.YkCode_ != RETURN_CODE_KEYERROR)
				{
					...
					{
						AddWaitCmdVal(YK_OVER_CON,YK_OVER_PRIORITY,pristationPtr,(int)(yk_para).YkNo_);
					}
	...
	case CALL_ALL_DATA_CON:
		...
		if (bytesAssemble > 0)
		{
			...
			{
				AddWaitCmdVal(ALL_YX_DATA,ALL_YX_DATA_PRIORITY,pristationPtr,(int)0);
			}
			else if (pristationPtr->getYcSum() > 0)
			{
				AddWaitCmdVal(ALL_YC_DATA,ALL_YC_DATA_PRIORITY,pristationPtr,(int)0);
			}
			else
			{
				AddWaitCmdVal(CALL_ALL_DATA_OVER,CALL_ALL_DATA_OVER_PRIORITY,pristationPtr);
			}
	...
	case CALL_ALL_YM_CON:
		...
		if (bytesAssemble > 0)
		{
			...
			{
				AddWaitCmdVal(ALL_YM_DATA,ALL_YM_DATA_PRIORITY,pristationPtr,(int)0);
			}
			else
			{
				AddWaitCmdVal(CALL_ALL_YM_OVER,CALL_ALL_YM_OVER_PRIORITY,pristationPtr);
			}
	...
	case ALL_YX_DATA:
		{
			...
			if (leftNum > 0)
			{
				...
				if (bytesAssemble > 0)
				{
					...
					{
						AddWaitCmdVal(ALL_YX_DATA,ALL_YX_DATA_PRIORITY,pristationPtr,startIndex);
					}
					else
					{
						if(pristationPtr->getYcSum() > 0)
	...
	case ALL_YC_DATA:
		{
			...
			if (leftNum > 0)
			{
				...
				if (bytesAssemble > 0)
				{
					...
					{
						AddWaitCmdVal(ALL_YC_DATA,ALL_YC_DATA_PRIORITY,pristationPtr,startIndex);
					}
					else
					{
						AddWaitCmdVal(CALL_ALL_DATA_OVER,CALL_ALL_DATA_OVER_PRIORITY,pristationPtr);
	...
	case ALL_YM_DATA:
		{
			...
			if (leftNum > 0)
			{
				...
				if (bytesAssemble > 0)
				{
					...
					{
						AddWaitCmdVal(ALL_YM_DATA,ALL_YM_DATA_PRIORITY,pristationPtr,startIndex);
					}
					else
					{
						AddWaitCmdVal(CALL_ALL_YM_OVER,CALL_ALL_YM_OVER_PRIORITY,pristationPtr);
...
int CS101::AssembleFrameTail( size_t bufBegin, size_t bufIndex, unsigned char * buf, CCmd & cmd )
{
...
void CS101::InitDefaultStartAddr()
{
...
void CS101::InitDefaultFrameElem()
{
...
void CS101::InitFrameLocation(size_t frameHead)
{
...
void CS101::InitDefaultTimer(boost::asio::io_service & io_service)
{
	InitDefaultTimeOut();
...
void CS101::handle_timerYkSel(const boost::system::error_code& error,share_commpoint_ptr point,size_t yk_no)
{
	if (!error)
	{
		if (point)
		{
			AddStatusLogWithSynT("S101规约遥控选择命令超时。\n");
...
void CS101::handle_timerYkExe(const boost::system::error_code& error,share_commpoint_ptr point,size_t yk_no)
{
	if (!error)
	{
		if (point)
		{
			AddStatusLogWithSynT("S101规约遥控执行命令超时。\n");
...
void CS101::handle_timerYkCancel(const boost::system::error_code& error,share_commpoint_ptr point,size_t yk_no)
{
	if (!error)
	{
		if (point)
		{
			AddStatusLogWithSynT("S101规约遥控取消命令超时。\n");
...
void CS101::handle_timerYkSelToExe(const boost::system::error_code& error,share_commpoint_ptr point,size_t yk_no)
{
	if (!error)
	{
		if (point)
...
void CS101::handle_timerCallPara(const boost::system::error_code& error,share_commpoint_ptr point)
{
	if (!error)
	{
		if (point)
...
void CS101::handle_timerSetPara(const boost::system::error_code& error,share_commpoint_ptr point)
{
	if (!error)
	{
		if (point)
...
void CS101::ResetTimerYkSel(share_commpoint_ptr point,size_t yk_no,bool bContinue /*= false*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerYkSel_->async_wait(boost::bind(&CS101::handle_timerYkSel,this,boost::asio::placeholders::error,point,yk_no));
	}
	else
	{
		timerYkSel_->cancel();
...
void CS101::ResetTimerYkExe(share_commpoint_ptr point,size_t yk_no,bool bContinue /*= false*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerYkExe_->async_wait(boost::bind(&CS101::handle_timerYkExe,this,boost::asio::placeholders::error,point,yk_no));
	}
	else
	{
		timerYkExe_->cancel();
...
void CS101::ResetTimerYkCancel(share_commpoint_ptr point,size_t yk_no,bool bContinue /*= false*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerYkCancel_->async_wait(boost::bind(&CS101::handle_timerYkCancel,this,boost::asio::placeholders::error,point,yk_no));
	}
	else
	{
...
void CS101::ResetTimerCallPara(share_commpoint_ptr point,bool bContinue /*= false*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerCallPara_->async_wait(boost::bind(&CS101::handle_timerCallPara,this,boost::asio::placeholders::error,point));
	}
	else
	{
...
void CS101::ResetTimerSetPara(share_commpoint_ptr point,bool bContinue /*= false*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerSetPara_->async_wait(boost::bind(&CS101::handle_timerSetPara,this,boost::asio::placeholders::error,point));
	}
	else
	{
...
void CS101::ResetTimerYkSelToExe(share_commpoint_ptr point,size_t yk_no,bool bContinue /*= false*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerYkSelToExe_->async_wait(boost::bind(&CS101::handle_timerYkSelToExe,this,boost::asio::placeholders::error,point,yk_no));
	}
	else
	{
...
//para api
int CS101::setSYX_START_ADDR(size_t val)
{
	if (val < 0 || val >= 0x4001)
	{
		return -1;
...
int CS101::setDYX_START_ADDR(size_t val)
{
	if (val < 0 || val >= 0x4001)
	{
		return -1;
...
int CS101::setYC_START_ADDR(size_t val)
{
...
int CS101::setSYK_START_ADDR(size_t val)
{
	if (val < 0x701)
	{
		return -1;
...
int CS101::setDYK_START_ADDR(size_t val)
{
	if (val < 0x701)
	{
		return -1;
...
int CS101::setYM_START_ADDR(size_t val)
{
	if (val < 0x701)
	{
		return -1;
...
int CS101::setHIS_START_ADDR(size_t val)
{
	HIS_START_ADDR_ = val;
...
int CS101::setFrameTypeLength(unsigned short val)
{
	if (val <= 0 || val > 4)
	{
		return -1;
...
int CS101::setInfoNumLength(unsigned short val)
{
	if (val <= 0 || val > 4)
	{
		return -1;
...
int CS101::setTransReasonLength(unsigned short val)
{
	if (val <= 0 || val > 8)
	{
		return -1;
...
int CS101::setAsduAddrLength(unsigned short val)
{
	if (val <= 0 || val > 8)
	{
		return -1;
...
int CS101::setInfoAddrLength(unsigned short val)
{
	if (val <= 0 || val > 12)
	{
		return -1;
...
int CS101::setTimeOutYkSel(unsigned short val)
{
	if (val <= 0 || val > 300)
	{
		return -1;
...
int CS101::setTimeOutYkExe(unsigned short val)
{
	if (val <= 0 || val > 300)
	{
		return -1;
...
int CS101::setTimeOutYkCancel(unsigned short val)
{
	if (val <= 0 || val > 300)
	{
		return -1;
...
int CS101::setTimeOutCallPara(unsigned short val)
{
	if (val <= 0 || val > 300)
	{
		return -1;
...
int CS101::setTimeOutSetPara(unsigned short val)
{
	if (val <= 0 || val > 300)
	{
		return -1;
...
int CS101::CreateYkFramePara(bool bCancel,bool bDouble,unsigned char yk_type,unsigned char return_code,unsigned char & yk_code,unsigned char & trans_reason)
{
