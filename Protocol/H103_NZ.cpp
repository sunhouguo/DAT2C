...
int CH103_NZ::LoadXmlCfg(std::string filename)
{
	...
	{
		std::cout<<"导入H103_NZCfg.xml配置文件"<<filename<<"失败！"<<std::endl;
...
int CH103_NZ::ParseFrameBody(unsigned char * buf,size_t exceptedBytes)
{
	...
	{
		std::ostringstream ostr;
		ostr<<"H103_NZ规约不能根据接收报文中的地址匹配terminal ptr,这帧报文将不会被解析。"<<std::endl;
...
int CH103_NZ::AssembleFrameBody(size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
	...
	{
		std::ostringstream ostr;
		ostr<<"H103_NZ规约不能从发送命令中获得terminal ptr，cmdtype = "<<cmd.getCmdType()<<"，这个命令将不会被发送。"<<std::endl;
		AddStatusLogWithSynT(ostr.str());
...
int CH103_NZ::InitProtocol()
{
	...
	AddStatusLogWithSynT("H103_NZ规约的通道打开成功。\n");
...
void CH103_NZ::UninitProtocol()
{
	...
	AddStatusLogWithSynT("H103_NZ规约的通道关闭成功。\n");
}
...
void CH103_NZ::handle_timerYkSel(const boost::system::error_code& error,share_commpoint_ptr point,size_t yk_no)
{
	if (!error)
	{
		if (point)
		{
			...
			AddStatusLogWithSynT("H103_NZ规约遥控选择命令超时。\n");
		}
	}
}
...
int CH103_NZ::ParseDoubleProtectYxWithTE(unsigned char * buf, share_terminal_ptr terminalPtr)
{
	...
	if (info_num <= 0)
	{
		AddStatusLogWithSynT("收到双点保护SOE报文，但是信息体数目错误，不处理该报文。\n" );
		return -1;
	}
		...
		for (int i=0;i<info_num;i++)
		{
			if (info_addr + i>= 0 && info_addr + i< (int)terminalPtr->getRecvYxSum())
			{
				...
				if (InfoDataLength >= 12)
				{
					...
					ostr<<"收到双点保护SOE，点号"<<info_addr+i<<"，YX值"<<std::hex<<std::uppercase<<std::setw(2)<<std::setfill('0')<<(short)YxBitVal<<"："<<to_simple_string(timeVal)<<std::endl;
				}
				else
				{
					ptime timeVal(lt.date(),td);
		...
		for (int i=0;i<info_num;i++)
		{
			...
			if (info_addr >= 0 && info_addr < (int)terminalPtr->getRecvYxSum())
			{
				...
				if (InfoDataLength >= InfoAddrLength_ + 12)
				{
					...
					ostr<<"收到双点保护SOE，点号"<<info_addr<<"，YX值"<<std::hex<<std::uppercase<<std::setw(2)<<std::setfill('0')<<(short)YxBitVal<<"："<<to_simple_string(timeVal)<<std::endl;
				}
				else
				{
					ptime timeVal(lt.date(),td);
			...
			{
				AddStatusLogWithSynT("收到双点保护SOE，但是信息体地址错误，不处理该点。\n");
			}
		}
	}
...
//				int info_addr = BufToVal(&buf[InfoAddrLocation_],InfoAddrLength_) - 241 + YCcount;
				...
				if (info_addr >= 0 && info_addr < (int)terminalPtr->getYcSum())
				{
					...
					/*ostr<<"收到遥测，点号"<<info_addr<<"，YC值"<<std::hex<<std::uppercase<<std::setw(2)<<std::setfill('0')<<(short)yc<<std::endl;
					AddStatusLogWithSynT(ostr.str());*/
					if (ret == DataBase::CauseActiveData)
					{
						ChangeYCcount++;
	...
	{
		/*std::ostringstream ostr;
		ostr<<"总召报文遥测YC产生了ycvar，数目："<<ChangeYCcount<<std::endl;
		AddStatusLogWithSynT(ostr.str());*/
	...
	if (ChangeYXcount > 0)
	{
		...
		{
			std::ostringstream ostr;
			ostr<<"报文遥信YX产生了COS和SOE，数目："<<ChangeYXcount<<std::endl;
			AddStatusLogWithSynT(ostr.str());
		...
		{
			std::ostringstream ostr;
			ostr<<"总召报文遥信YX产生了cos，数目："<<ChangeYXcount<<std::endl;
			AddStatusLogWithSynT(ostr.str());
		}
