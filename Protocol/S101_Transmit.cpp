...
const std::string strDefaultCfg = "S101_TransmitCfg.xml";
...
CS101_Transmit::CS101_Transmit(boost::asio::io_service & io_service)
...
CS101_Transmit::~CS101_Transmit(void)
{
}

void CS101_Transmit::InitDefaultFrameElem()
{
...
void CS101_Transmit::InitFrameLocation(size_t frameHead)
{
...
int CS101_Transmit::MatchFrameHead( size_t & exceptedBytes )
{
...
int CS101_Transmit::CheckFrameHead(unsigned char * buf,size_t & exceptedBytes)
{
	if(CheckPubKey())
	{
...
int CS101_Transmit::CheckFrameTail(unsigned char * buf,size_t exceptedBytes)
{
	return 0;
}

int CS101_Transmit::ParseSynTime(unsigned char * buf,share_pristation_ptr pristationPtr)
{
...
int CS101_Transmit::ParseYkSign(unsigned char * buf,share_pristation_ptr pristationPtr,size_t exceptedBytes)
{
	int OriFrameLength = buf[1] + 6;
...
int CS101_Transmit::ParseFrameBody(unsigned char * buf,size_t exceptedBytes)
{
	...
	if(CheckPubKey() && exceptedBytes > 5)
	{
		size_t FrameType = BufToVal(&buf[FrameTypeLocation_],FrameTypeLength_);        //类型标识
...
int CS101_Transmit::AssembleFrameHead(size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
...
int CS101_Transmit::AssembleFrameBody(size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
	size_t count = bufIndex;
	...
	case TRANSMIT_FRAME:
		{
			...
			{
				std::cerr<<"CS101_Transmit::AssembleFrameBody "<<e.what()<<std::endl;
				return -1;
			}
...
int CS101_Transmit::AssembleFrameTail(size_t bufBegin, size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
	size_t count = bufIndex;
...
int CS101_Transmit::LoadXmlCfg(std::string filename)
{
...
void CS101_Transmit::SaveXmlCfg(std::string filename)
{
	FileSystem::CMarkup xml;
...
int CS101_Transmit::setFrameTypeLength(unsigned short val)
{
	if (val <= 0 || val > 4)
	{
		return -1;
...
int CS101_Transmit::setInfoNumLength(unsigned short val)
{
	if (val <= 0 || val > 4)
	{
		return -1;
...
int CS101_Transmit::setTransReasonLength(unsigned short val)
{
	if (val <= 0 || val > 8)
	{
		return -1;
...
int CS101_Transmit::setAsduAddrLength(unsigned short val)
{
	if (val <= 0 || val > 8)
	{
		return -1;
...
int CS101_Transmit::setInfoAddrLength(unsigned short val)
{
	if (val <= 0 || val > 12)
	{
		return -1;
...
void CS101_Transmit::ProcessSubAliveSig(typeCmd cmdType,unsigned char ReturnCode,share_commpoint_ptr point,boost::any val)
{
	switch (cmdType)
	{
	case TRANSMIT_FRAME:
...
int CS101_Transmit::InitProtocol()
{
	ConnectSubAliveSig();
	...
	AddStatusLogWithSynT("S101_Transmit规约的通道打开成功。\n");

	return 0;
}

void CS101_Transmit::UninitProtocol()
{
	DisconnectSubAliveSig();

	CProtocol::UninitProtocol();

	AddStatusLogWithSynT("S101_Transmit规约的通道关闭成功。\n");
}
