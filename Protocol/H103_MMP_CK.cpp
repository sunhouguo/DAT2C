...
const std::string strDefaultCfg = "H103_MMP_CKCfg.xml";
size_t CH103_MMP_CK::H103ObjectCounter_ = 1;
...
CH103_MMP_CK::CH103_MMP_CK(boost::asio::io_service & io_service)
...
CH103_MMP_CK::~CH103_MMP_CK(void)
{
	H103ObjectCounter_--;
}

int CH103_MMP_CK::LoadXmlCfg(std::string filename)
{
...
void CH103_MMP_CK::SaveXmlCfg(std::string filename)
{
	FileSystem::CMarkup xml;
...
int CH103_MMP_CK::CheckFrameHead(unsigned char * buf,size_t & exceptedBytes)
{
...
int CH103_MMP_CK::CheckFrameTail(unsigned char * buf,size_t exceptedBytes)
{
	size_t sum = 0;

	//if ((exceptedBytes > 5) && (buf[0] == 0x68) && (buf[3] == 0x68))
	//{
		sum = CalcCheckSumByte(&buf[0],exceptedBytes - 1);
	//}

	if (buf[exceptedBytes - 1] == sum)
	{
		return 0;
	}
...
int CH103_MMP_CK::ParseFrameBody(unsigned char * buf,size_t exceptedBytes)
{
	int ret = 0;
	...
	size_t FrameType = BufToVal(&buf[2],1);
	switch(FrameType)
	{
	case C_IGI_NA_3:
...
int CH103_MMP_CK::AssembleFrameHead(size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
...
int CH103_MMP_CK::AssembleFrameBody(size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
	int bytesAssemble = 0;
...
int CH103_MMP_CK::AssembleFrameTail(size_t bufBegin, size_t bufIndex, unsigned char * buf, CCmd & cmd)
{
...
int CH103_MMP_CK::InitProtocol()
{
	...
	AddStatusLogWithSynT("H103_MMP_CK规约的通道打开成功。\n");

	return 0;
}

void CH103_MMP_CK::UninitProtocol()
{
	CProtocol::UninitProtocol();

	AddStatusLogWithSynT("H103_MMP_CK规约的通道关闭成功。\n");
}

void CH103_MMP_CK::InitObjectIndex()
{
	ProtocolObjectIndex_ = H103ObjectCounter_++;
}

void CH103_MMP_CK::InitDefaultTimeOut()
{
...
void CH103_MMP_CK::InitDefaultTimer(boost::asio::io_service & io_service)
{
	InitDefaultTimeOut();
...
int CH103_MMP_CK::AssembleSynTime(size_t bufIndex, unsigned char * buf, share_terminal_ptr terminalPtr, boost::posix_time::ptime time)
{
	size_t count = bufIndex;
	...
	int var = (int)(td.total_milliseconds() % MinutesRemainderMillisecs)/5;
	count += ValToBuf(&buf[count],var,1);


	return count - bufIndex;
}

int CH103_MMP_CK::AssembleCallAllData(size_t bufIndex, unsigned char * buf, share_terminal_ptr terminalPtr)
{
	size_t count = bufIndex;
...
int CH103_MMP_CK::AssembleYKSel( size_t bufIndex, unsigned char * buf, share_terminal_ptr terminalPtr,unsigned char yk_obj, unsigned char yk_code )
{
	size_t count = bufIndex;

	count += ValToBuf(&buf[count],terminalPtr->getAddr(),1);
	count += ValToBuf(&buf[count],5,1);
...
int CH103_MMP_CK::AssembleYKExe( size_t bufIndex, unsigned char * buf, share_terminal_ptr terminalPtr,unsigned char yk_obj, unsigned char yk_code )
{
	size_t count = bufIndex;
...
int CH103_MMP_CK::ParseAllData(unsigned char * buf, share_terminal_ptr terminalPtr)
{
	int DataLocation = 4;
	int frameLength = buf[2] - 6;

	size_t count = 0;
	if (frameLength - DataLocation >= 18*2)
	{
		for (int i=0;i<18;i++)
		{
	...
	if (frameLength - DataLocation >= 4)
	{
		...
		int mecs = buf[DataLocation++] * 5;
	}

	return 0;
}

int CH103_MMP_CK::ParseAllEvent(unsigned char * buf, share_terminal_ptr terminalPtr)
{
	int Yx_val = 0;
	int DataLocation = 4;
	int frameLength = buf[2] - 6;
	...
	int count = 0;
	int inf_no = frameLength/8;
	for (int i=1;i<inf_no;i++)
	{
		...
		int sec = buf[DataLocation++];
		int mecs = buf[DataLocation++]*5;
		int Yx_no = buf[DataLocation++];
		int val = buf[DataLocation++];
		...
		terminalPtr->SaveYxType(Yx_no+i,DataBase::single_yx_point);
		int ret = terminalPtr->SaveOriYxVal(Yx_no,Yx_val,terminalPtr->getInitCommPointFlag());
		if (ret == DataBase::CauseActiveData)
		{
			count++;
		}
		DataLocation = DataLocation+2;
...
int CH103_MMP_CK::ParseYkSelCon(unsigned char * buf, share_terminal_ptr terminalPtr)
{
	int DataLocation = 4;
...
void CH103_MMP_CK::handle_timerCallAllData(const boost::system::error_code& error,share_commpoint_ptr point)
{
	if (!error)
	{
...
void CH103_MMP_CK::handle_timerSynTime(const boost::system::error_code& error,share_commpoint_ptr point)
{
	if (!error)
	{
		share_commpoint_ptr val = getNextCommPoint(TERMINAL_NODE,true,point);
...
void CH103_MMP_CK::handle_timerYkSel(const boost::system::error_code& error,share_commpoint_ptr point,size_t yk_no)
{
	if (!error)
	{
		if (point)
...
void CH103_MMP_CK::ResetTimerCallAllData(share_commpoint_ptr point,bool bContinue /*= true*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerCallAllData_->async_wait(boost::bind(&CH103_MMP_CK::handle_timerCallAllData,this,boost::asio::placeholders::error,point));
	}
	else
	{
...
void CH103_MMP_CK::ResetTimerSynTime(share_commpoint_ptr point,bool bContinue /*= true*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerSynTime_->async_wait(boost::bind(&CH103_MMP_CK::handle_timerSynTime,this,boost::asio::placeholders::error,point));
	}
	else
	{
...
void CH103_MMP_CK::ResetTimerYkSel(share_commpoint_ptr point,size_t yk_no,bool bContinue /*= false*/,unsigned short val/* = 0*/)
{
	if (bContinue)
	{
		if (val == 0)
		...
		timerYkSel_->async_wait(boost::bind(&CH103_MMP_CK::handle_timerYkSel,this,boost::asio::placeholders::error,point,yk_no));
	}
	else
	{
		timerYkSel_->cancel();
...
int CH103_MMP_CK::TransYkCmdToPara(int yk_no,typeYktype yk_type,unsigned char & yk_obj,unsigned char & yk_code)
{
	int ret = -1;
	bool yk_noval = 1;
	...
	if (yk_noval)
	{
		if (yk_type == DataBase::YkOpen)
		{
			yk_code = 0x33;
			ret = 0;
		}
		else if (yk_type == DataBase::YkClose)
		{
			yk_code = 0x55;
			ret = 0;
		}
...
int CH103_MMP_CK::TransYkParaToCmd(unsigned char yk_obj,unsigned char yk_code,int & yk_no,typeYktype & yk_type)
{
	int ret = -1;
	bool yk_noval = 0;
